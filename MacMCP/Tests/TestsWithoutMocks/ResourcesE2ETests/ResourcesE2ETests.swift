...
struct ResourcesE2ETests {
    ...
    private mutating func setUp() async throws {
        ...
        let runningApps = NSRunningApplication.runningApplications(withBundleIdentifier: calculatorBundleId)
    ...
    @Test("Test applications resource lists Calculator")
    mutating func testApplicationsResource() async throws {
        try await setUp()
        ...
        let handler = ApplicationsResourceHandler(applicationService: applicationService, logger: logger)
    ...
    @Test("Test UI element resource for Calculator application")
    mutating func testUIElementResource() async throws {
        try await setUp()
    ...
    mutating func testInteractableElementsFiltering() async throws {
        try await setUp()
    ...
    mutating func testResourceRegistryWithRealHandlers() async throws {
        ...
        let applicationsHandler = ApplicationsResourceHandler(applicationService: applicationService, logger: logger)
        let uiElementHandler = UIElementResourceHandler(accessibilityService: accessibilityService, logger: logger)
        ...
        #expect(resources.contains(where: { $0.id.contains("applications") }), "Should contain applications resource")
        #expect(resources.contains(where: { $0.id.contains("ui") }), "Should contain UI elements resource")
