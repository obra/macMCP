...
struct ChangeDetectionE2ETest {
    ...
    private mutating func setUp() async throws {
        ...
        (logger, logFileURL) = TestLogger.create(label: "mcp.test.change_detection", testName: "ChangeDetectionE2ETest")
    ...
    @Test("UI Change Detection with Button Click")
    mutating func testChangeDetectionButtonClick() async throws {
        try await setUp()
        ...
        // Detect changes
        let changes = changeDetectionService.detectChanges(
            before: beforeSnapshot,
            after: afterSnapshot
        )
        
        // Verify that changes were detected
        let totalChanges = changes.newElements.count + changes.removedElements.count + changes.modifiedElements.count
        logger.info("Detected \(totalChanges) total changes: \(changes.newElements.count) new, \(changes.removedElements.count) removed, \(changes.modifiedElements.count) modified")
        
        // Dump detailed changes for inspection
        logger.info("=== NEW ELEMENTS ===")
        for element in changes.newElements {
            logger.info("NEW: \(element.path) - \(element.role) '\(element.title ?? "")' value='\(element.value ?? "")'")
        }
        ...
        for change in changes.modifiedElements.prefix(5) {
            ...
            if change.before.description != change.after.description {
                changes.append("description: '\(change.before.description ?? "")' → '\(change.after.description ?? "")'")
            }
            ...
            if change.before.identifier != change.after.identifier {
                changes.append("identifier: '\(change.before.identifier ?? "")' → '\(change.after.identifier ?? "")'")
            }
            ...
            if change.before.children.count != change.after.children.count {
                changes.append("children count: \(change.before.children.count) → \(change.after.children.count)")
            }
            ...
            if changes.isEmpty {
                ...
                if changes.count > 3 {
                    logger.info("  ... and \(changes.count - 3) more changes")
                }
            }
        }
        ...
        try await calculatorHelper.assertDisplayValue("1", message: "Calculator should show '1' after clicking button")
        
        try await tearDown()
    }
    
    /// Test that UI change detection works with keyboard input
    @Test("UI Change Detection with Keyboard Input")
    mutating func testChangeDetectionKeyboardInput() async throws {
        try await setUp()
        ...
        try await calculatorHelper.assertDisplayValue("42", message: "Calculator should show '42' after typing")
    ...
    mutating func testChangeDetectionService() async throws {
        try await setUp()
    ...
    mutating func testChangeDetectionMenuNavigation() async throws {
        ...
        let aboutItems = menuItems.filter { item in
            if case .text(let itemText) = item {
                return itemText.lowercased().contains("about")
            }
            return false
        }
        ...
        if let aboutItem = aboutItems.first {
            ...
            if menuResponse.contains("\"id\":") {
                ...
                logger.info("Menu navigation structure verified - skipping actual activation to avoid modal dialogs")
            }
        } else {
            logger.info("No About menu found - this is OK, just testing the change detection infrastructure")
        }
