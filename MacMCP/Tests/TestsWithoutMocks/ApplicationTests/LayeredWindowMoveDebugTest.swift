...
@Suite(.serialized)
struct LayeredWindowMoveDebugTest {
  ...
  private mutating func setUp() async throws {
    // Set up logging
    (logger, _) = TestLogger.create(label: "mcp.test.layered_debug", testName: "LayeredWindowMoveDebugTest")
    ...
    textEditApp = NSWorkspace.shared.runningApplications.first(where: { $0.bundleIdentifier == "com.apple.TextEdit" })
    if textEditApp == nil {
    ...
    guard textEditApp != nil else {
      throw TestError.setupFailed("TextEdit could not be launched")
    }
  ...
  @Test("Layered window movement debug")
  mutating func testLayeredWindowMovement() async throws {
    try await setUp()
    
    guard let app = textEditApp else {
      throw TestError.setupFailed("TextEdit app not available")
    }
    ...
    let directMoveResult = AXUIElementSetAttributeValue(directWindow, kAXPositionAttribute as CFString, pointValue1)
    logger.info("   📊 Direct move result: \(directMoveResult) (\(directMoveResult == .success ? "SUCCESS" : "FAILURE"))")
    
    if directMoveResult != .success {
      throw TestError.testFailed("LAYER 1 FAILED: Direct AX API movement failed with \(directMoveResult)")
    }
    
    // ==== LAYER 2: ElementPath.resolve() ====
    logger.info("\n📍 LAYER 2: ElementPath.resolve()")
    
    let pathString = "macos://ui/AXApplication[@AXTitle=\"TextEdit\"][@bundleId=\"com.apple.TextEdit\"]/AXWindow[@AXTitle=\"\(directTitle)\"]"
    ...
    do {
      ...
      throw TestError.testFailed("LAYER 2 FAILED: ElementPath parsing failed: \(error)")
    }
    ...
    logger.info("   📊 Element ID comparison: Direct=\(directElementID), Resolved=\(resolvedElementID)")
    ...
    let resolvedMoveResult = AXUIElementSetAttributeValue(resolvedElement, kAXPositionAttribute as CFString, pointValue2)
    logger.info("   📊 Resolved element move result: \(resolvedMoveResult) (\(resolvedMoveResult == .success ? "SUCCESS" : "FAILURE"))")
    
    if resolvedMoveResult != .success {
      logger.error("   ❌ LAYER 2 FAILED: ElementPath resolved element movement failed with \(resolvedMoveResult)")
    }
    ...
    // ==== FINAL ANALYSIS ====
    logger.info("\n📊 FINAL ANALYSIS:")
    logger.info("   Layer 1 (Raw AX API): \(directMoveResult == .success ? "✅ SUCCESS" : "❌ FAILED")")
    logger.info("   Layer 2 (ElementPath): \(resolvedMoveResult == .success ? "✅ SUCCESS" : "❌ FAILED")")
    logger.info("   Element ID Match: \(directElementID == resolvedElementID ? "✅ SAME" : "❌ DIFFERENT")")
