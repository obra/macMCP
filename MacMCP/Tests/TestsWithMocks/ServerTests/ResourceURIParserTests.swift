...
@Suite(.serialized) struct ResourceURIParserTests {
  @Test("Test parsing simple URI with scheme") func parseSimpleURIWithScheme() throws {
  ...
  func parseURIWithPathComponentsAndScheme() throws {
  ...
  @Test("Test parsing URI without scheme") func parseURIWithoutScheme() throws {
  ...
  @Test("Test parsing URI with query parameters") func parseURIWithQueryParameters() throws {
  ...
  @Test("Test parsing URI with leading slash") func parseURIWithLeadingSlash() throws {
    let uri = "macos:///applications/com.apple.TextEdit/windows"
    let components = try ResourceURIParser.parse(uri)
    #expect(components.scheme == "macos", "Scheme should be 'macos'")
    #expect(
  ...
  private struct MockResourceHandler: ResourceHandler {
    ...
    func handleRead(uri _: String, components _: ResourceURIComponents) async throws -> (
      ResourcesRead.ResourceContent, ResourcesRead.ResourceMetadata?
    ) { (.text("{}"), ResourcesRead.ResourceMetadata(mimeType: "application/json")) }
  }
}
