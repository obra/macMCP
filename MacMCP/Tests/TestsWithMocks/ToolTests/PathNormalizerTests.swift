...
struct PathNormalizerTests {
    ...
    func testNormalizeAttributeNames() {
    ...
    func testEscapeAttributeValues() {
        // Test quote escaping
        #expect(PathNormalizer.escapeAttributeValue("Value with \"quotes\"") == "Value with \\\"quotes\\\"")
        
        // Test backslash escaping
        #expect(PathNormalizer.escapeAttributeValue("Value with \\ backslash") == "Value with \\\\ backslash")
        
        // Test control character escaping
        #expect(PathNormalizer.escapeAttributeValue("Value with \n newline") == "Value with \\n newline")
        ...
        #expect(PathNormalizer.escapeAttributeValue("\"Value\"\nwith\\everything") == "\\\"Value\\\"\\nwith\\\\everything")
    }
    
    @Test("Unescape attribute values")
    func testUnescapeAttributeValues() {
        // Test quote unescaping
        #expect(PathNormalizer.unescapeAttributeValue("Value with \\\"quotes\\\"") == "Value with \"quotes\"")
        
        // Test backslash unescaping
        #expect(PathNormalizer.unescapeAttributeValue("Value with \\\\ backslash") == "Value with \\ backslash")
        
        // Test control character unescaping
        #expect(PathNormalizer.unescapeAttributeValue("Value with \\n newline") == "Value with \n newline")
        #expect(PathNormalizer.unescapeAttributeValue("Value with \\t tab") == "Value with \t tab")
        #expect(PathNormalizer.unescapeAttributeValue("Value with \\r return") == "Value with \r return")
        
        // Test combined unescaping
        #expect(PathNormalizer.unescapeAttributeValue("\\\"Value\\\"\\nwith\\\\everything") == "\"Value\"\nwith\\everything")
    }
    
    @Test("Round-trip escaping and unescaping")
    func testRoundTripEscaping() {
        let testCases = [
            "Simple string",
            "String with \"quotes\"",
    ...
    func testNormalizePathString() {
        ...
        let path2 = "ui://AXWindow/AXGroup[@AXTitle=\"Main\"][@id=\"mainGroup\"]/AXButton[@value=\"Click\"]"
        ...
        let path3 = "ui://AXWindow/AXGroup[@AXTitle=\"Already Normalized\"]/AXButton[@AXEnabled=\"false\"]"
