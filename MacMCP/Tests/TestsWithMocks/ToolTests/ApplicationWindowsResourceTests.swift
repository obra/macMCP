...
@Suite(.serialized)
struct ApplicationWindowsResourceTests {
    ...
    class TestableApplicationWindowsResourceHandler: ApplicationWindowsResourceHandler, @unchecked Sendable {
    ...
    final class MockAccessibilityService: @unchecked Sendable, AccessibilityServiceProtocol {
        ...
        init(windowElements: [MockUIElement] = []) {
    ...
    @Test("Test handling application windows resource with no windows")
    func testNoWindows() async throws {
        ...
        if let metadata = metadata {
            ...
            #expect(metadata.additionalMetadata?["bundleId"]?.stringValue == "com.apple.calculator", "Bundle ID should be correct")
            #expect(metadata.additionalMetadata?["windowCount"]?.doubleValue == 0, "Window count should be 0")
        }
    }
    ...
    func testMultipleWindows() async throws {
        ...
        if let metadata = metadata {
            #expect(metadata.mimeType == "application/json", "MIME type should be application/json")
            #expect(metadata.additionalMetadata?["bundleId"]?.stringValue == "com.test.app", "Bundle ID should be correct")
            #expect(metadata.additionalMetadata?["windowCount"]?.doubleValue == 3, "Window count should be 3")
        }
    }
    ...
    func testFilteringMinimizedWindows() async throws {
        ...
        #expect(metadata != nil, "Metadata should be provided")
        if let metadata = metadata {
            #expect(metadata.additionalMetadata?["windowCount"]?.doubleValue == 1, "Window count should be 1")
        }
    }
    
    @Test("Test handling application windows resource with error")
    func testErrorHandling() async throws {
        ...
        // Create the resource handler
        let handler = TestableApplicationWindowsResourceHandler(accessibilityService: errorService, logger: logger)
        ...
        do {
            ...
            #expect(error.localizedDescription.contains("permission denied"), "Should throw a permission denied error")
        }
    }
    
    @Test("Test invalid URI params")
    func testInvalidURIParams() async throws {
        let mockService = MockAccessibilityService()
        let logger = Logger(label: "test.application.windows")
        ...
        } catch _ as ResourceURIError {
            #expect(Bool(true), "Should throw a ResourceURIError")
        } catch {
