...
@Suite(.serialized) struct ResourcesUIElementTests {
  ...
  final class MockAccessibilityService: @unchecked Sendable, AccessibilityServiceProtocol {
    ...
    func findElementByPath(path: String) async throws -> UIElement? { return rootElement }
  ...
  @Test("Test basic UI element resource") func testBasicUIElementResource() async throws {
  ...
  @Test("Test error handling for invalid path") func testErrorHandlingForInvalidPath() async throws
  {
    ...
    do {
      ...
      switch error {
      case .invalidParams: #expect(true, "Should throw invalidParams error")
      default: #expect(false, "Should throw invalidParams error, got \(error)")
      }
    }
  }
