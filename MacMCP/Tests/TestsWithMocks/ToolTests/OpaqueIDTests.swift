...
@Suite("Opaque ID Encoder Tests")
struct OpaqueIDTests {
  
  @Test("Basic encoding and decoding")
  func basicEncodingDecoding() throws {
    let originalPath = #"macos://ui/AXApplication[@AXTitle="Calculator"]/AXButton[@AXDescription="1"]"#
  ...
  func complexPathEncoding() throws {
    let complexPath = #"macos://ui/AXApplication[@AXTitle="Calculator"][@bundleId="com.apple.calculator"]/AXWindow[@AXTitle="Calculator"]/AXGroup/AXSplitGroup/AXGroup/AXGroup/AXButton[@AXDescription="All Clear"]"#
    
    let opaqueID = try OpaqueIDEncoder.encode(complexPath)
    let decodedPath = try OpaqueIDEncoder.decode(opaqueID)
    
    #expect(decodedPath == complexPath)
    print("Complex path compression ratio: \(String(format: "%.1f", Double(complexPath.count) / Double(opaqueID.count)))x")
  }
  
  @Test("Special characters handling")
  func specialCharactersHandling() throws {
    let pathWithSpecialChars = #"macos://ui/AXApplication[@AXTitle="My \"Special\" App"]/AXButton[@AXDescription="Click & Save"]"#
