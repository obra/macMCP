...
@Suite("Opaque ID Mapper Tests") struct OpaqueIDMapperTests {
  @Test("Basic mapping functionality") func basicMapping() throws {
  ...
  @Test("Consistent ID generation") func consistentIDGeneration() throws {
    let mapper = OpaqueIDMapper(maxCapacity: 1000)
    let testPath = #"macos://ui/AXApplication[@AXTitle="Calculator"]/AXButton[@AXDescription="1"]"#
  ...
  @Test("Different paths get different IDs") func differentPathsDifferentIDs() throws {
    let mapper = OpaqueIDMapper(maxCapacity: 1000)
  ...
  @Test("LRU cache eviction") func lruCacheEviction() throws {
  ...
  @Test("LRU access order update") func lruAccessOrderUpdate() throws {
    let mapper = OpaqueIDMapper(maxCapacity: 3)
    let id1 = mapper.opaqueID(for: "path1")
    let id2 = mapper.opaqueID(for: "path2")
    let id3 = mapper.opaqueID(for: "path3")
  ...
  @Test("Clear all functionality") func clearAllFunctionality() throws {
    let mapper = OpaqueIDMapper(maxCapacity: 1000)
    let id1 = mapper.opaqueID(for: "path1")
    let id2 = mapper.opaqueID(for: "path2")
    #expect(mapper.cacheSize == 2)
  ...
  @Test("Thread safety basic test") func threadSafetyBasicTest() throws {
    let mapper = OpaqueIDMapper(maxCapacity: 1000)
    var results: [String] = []
    ...
    DispatchQueue.concurrentPerform(iterations: 100) { i in
      ...
      resultsLock.lock()
      if resolved == path { results.append(path) }
      resultsLock.unlock()
    }
  ...
  @Test("UUID encoding format validation") func uuidEncodingFormatValidation() throws {
    let mapper = OpaqueIDMapper(maxCapacity: 1000)
