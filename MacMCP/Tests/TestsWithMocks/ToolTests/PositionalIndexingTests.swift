...
@Suite(.serialized)
struct PositionalIndexingTests {
    
    @Test("insertPositionalIndex function - syntax formatting")
    func testInsertPositionalIndex() throws {
        ...
        #expect(indexed3 == "AXButton#1[@AXTitle=\"Save\"][@AXDescription=\"Save Document\"]", "Should insert #1 after role before first attribute")
    }
    
    @Test("Positional indexing algorithm - expected behavior")  
    func testPositionalIndexingAlgorithm() throws {
    ...
    @Test("Generated path formats with positional indexing")
    func testGeneratedPathFormats() throws {
        // Test that the final paths look correct
        let testCases = [
            (role: "AXButton", attrs: ["AXDescription": "Save"], index: nil, expected: "AXButton[@AXDescription=\"Save\"]"),
            (role: "AXButton", attrs: ["AXDescription": "Add"], index: 2, expected: "AXButton#2[@AXDescription=\"Add\"]"),
            (role: "AXButton", attrs: ["AXDescription": "Add"], index: 3, expected: "AXButton#3[@AXDescription=\"Add\"]"),
            (role: "AXButton", attrs: ["AXDescription": "Cancel"], index: nil, expected: "AXButton[@AXDescription=\"Cancel\"]"),
            (role: "AXButton", attrs: ["AXDescription": "Add"], index: 5, expected: "AXButton#5[@AXDescription=\"Add\"]"),
        ]
        
        for testCase in testCases {
            let basePath = createMockPath(role: testCase.role, attributes: testCase.attrs)
            let finalPath = if let index = testCase.index {
