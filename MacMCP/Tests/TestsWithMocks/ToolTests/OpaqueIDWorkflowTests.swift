...
// ABOUTME: Verifies that ElementDescriptor outputs opaque IDs and UIInteractionTool can decode them
...
struct OpaqueIDWorkflowTests {
  ...
  func endToEndOpaqueIDWorkflow() throws {
    // 1. Create an ElementDescriptor with a complex path (the problematic one from the user)
    ...
    let decodedDescriptor = try decoder.decode(ElementDescriptor.self, from: jsonData)
