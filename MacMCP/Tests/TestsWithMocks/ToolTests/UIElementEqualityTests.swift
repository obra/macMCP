...
@Suite(.serialized)
struct UIElementEqualityTests {
    
    @Test("UIElement equality basics - same path should be equal")
    func testBasicEquality() throws {
    ...
    @Test("Investigate impossible condition scenario")
    func testImpossibleCondition() throws {
        let element1 = createTestUIElement(path: "macos://ui/AXButton[@AXDescription=\"Test\"]")
        let element2 = createTestUIElement(path: "macos://ui/AXButton[@AXDescription=\"Test\"]")
    ...
    @Test("Test different objects with same path")
    func testDifferentObjectsSamePath() throws {
        ...
        #expect(element1 == element2, "Elements with same path should be equal regardless of other properties")
        #expect(!(element1 != element2), "Negation should be consistent")
    }
}
