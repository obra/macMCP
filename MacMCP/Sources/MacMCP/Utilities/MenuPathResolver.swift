...
public struct MenuPathResolver {
    ...
    public static func parsePath(_ path: String) -> [String] {
        return path.components(separatedBy: pathSeparator)
    ...
    public static func validatePath(_ path: String) -> Bool {
        ...
        if path.hasPrefix(pathSeparator) || path.hasSuffix(pathSeparator) {
            return false
        }
        
        // Check for consecutive separators
        if path.contains(pathSeparator + pathSeparator) {
            return false
        }
    ...
    public static func findPartialMatches(_ path: String, in hierarchy: MenuHierarchy) -> [String] {
        ...
        return allPaths.filter { 
            $0.lowercased().contains(lowercasePath)
        }.sorted { path1, path2 in
            // Prioritize exact prefix matches
            let path1Lower = path1.lowercased()
            let path2Lower = path2.lowercased()
            
            if path1Lower.hasPrefix(lowercasePath) && !path2Lower.hasPrefix(lowercasePath) {
                return true
            }
    ...
    public static func suggestSimilar(
        ...
        if !partialMatches.isEmpty {
            return Array(partialMatches.prefix(maxSuggestions))
        }
        ...
        for candidatePath in allPaths {
            ...
            if score > 0 {
                scoredPaths.append((candidatePath, score))
            }
        }
    ...
    private static func calculatePathSimilarity(_ components1: [String], _ components2: [String]) -> Int {
        var score = 0
