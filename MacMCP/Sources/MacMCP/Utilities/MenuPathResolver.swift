...
public enum MenuPathResolver {
  ...
  public static func buildPath(from menuTitles: [String]) -> String {
    menuTitles.joined(separator: pathSeparator)
  }
  ...
  public static func parsePath(_ path: String) -> [String] {
    path.components(separatedBy: pathSeparator).map {
  ...
  public static func suggestSimilar(
    ...
    return Array(scoredPaths.prefix(maxSuggestions).map(\.0))
  }
  ...
  private static func levenshteinDistance(_ string1: String, _ string2: String) -> Int {
    ...
    var matrix = Array(
      repeating: Array(repeating: 0, count: secondArray.count + 1),
      count: firstArray.count + 1,
    )
