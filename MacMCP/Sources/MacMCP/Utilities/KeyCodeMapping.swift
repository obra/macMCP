// ABOUTME: This file provides mappings between keys, characters, and key codes for keyboard interaction.
// ABOUTME: It handles the complexity of mapping characters to macOS virtual key codes and modifier flags.
...
public struct KeyCodeMapping {
    ...
    public static func keyCodeForKey(_ keyName: String) -> KeyCodeWithModifiers? {
        ...
        if let keyCode = numberKeyCodes[lowercaseKey] {
            return KeyCodeWithModifiers(keyCode: keyCode)
        }

        if let keyCode = specialKeyCodes[lowercaseKey] {
            return KeyCodeWithModifiers(keyCode: keyCode)
        }
        ...
        if let keyCode = symbolKeyCodes[lowercaseKey] {
            return KeyCodeWithModifiers(keyCode: keyCode)
        }
    ...
    public static func modifierFlagsForNames(_ modifierNames: [String]) -> CGEventFlags {
        ...
        for name in modifierNames {
            if let modifierFlag = modifierFlags[name.lowercased()] {
    ...
    public static func isModifierKey(_ keyName: String) -> Bool {
        return modifierKeyCodes.keys.contains(keyName.lowercased())
    }
    ...
    public static func cgEventFlagsFromNSModifierFlags(_ nsModifiers: NSEvent.ModifierFlags) -> CGEventFlags {
        ...
        if nsModifiers.contains(.function) {
    ...
    public static func nsModifierFlagsFromCGEventFlags(_ cgModifiers: CGEventFlags) -> NSEvent.ModifierFlags {
        var nsFlags: NSEvent.ModifierFlags = []
        
        if cgModifiers.contains(.maskCommand) {
            nsFlags.insert(.command)
        }
        
        if cgModifiers.contains(.maskShift) {
            nsFlags.insert(.shift)
        }
        
        if cgModifiers.contains(.maskAlternate) {
            nsFlags.insert(.option)
        }
        
        if cgModifiers.contains(.maskControl) {
            nsFlags.insert(.control)
        }
        
        if cgModifiers.contains(.maskSecondaryFn) {
