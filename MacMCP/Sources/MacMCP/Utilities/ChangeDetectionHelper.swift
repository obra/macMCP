...
public struct ChangeDetectionHelper {
    ...
    public static func extractChangeDetectionParams(_ params: [String: Value]) -> (detectChanges: Bool, delay: TimeInterval) {
    ...
    public static func formatResponse(message: String, uiChanges: UIChanges?, logger: Logger) -> [Tool.Content] {
        if let changes = uiChanges, changes.hasChanges {
            ...
            do {
                ...
                logger.warning("Failed to serialize UI changes response: \(error)")
            }
        }
    ...
    public static func addChangeDetectionSchemaProperties() -> [String: Value] {
        return [
            "detectChanges": .object([
                "type": .string("boolean"),
                "description": .string("Enable automatic UI change detection after interaction (default: true)"),
                "default": .bool(true),
            ]),
