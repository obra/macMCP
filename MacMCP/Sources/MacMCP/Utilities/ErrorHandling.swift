// ABOUTME: This file implements detailed error handling for MacMCP.
// ABOUTME: It provides consistent error formatting, categorization, and helpful recovery suggestions.
...
public struct MacMCPErrorInfo: Swift.Error, LocalizedError, Sendable {
    ...
    public var recoverySuggestion: String? {
        switch category {
        case .permissions:
            return "Check System Settings > Privacy & Security > Accessibility to ensure this application has the required permissions."
            
        case .accessibility:
            return "The macOS accessibility API encountered an issue. This may be due to system restrictions or unsupported UI elements."
            
        case .interaction:
            return "The UI element might not support this interaction, or the element might have changed or disappeared. Try getting the current UI state first."
            
        case .element:
            return "The UI element could not be found or accessed. It may have been removed, changed, or might be in a different part of the UI hierarchy."
            
        case .screenshot:
            return "Taking a screenshot failed. Make sure screen recording permissions are granted in System Settings > Privacy & Security."
            
        case .application:
            return "The target application could not be accessed. Make sure it is running and is not in a restricted mode."
            
        case .applicationLaunch:
            return "Failed to launch the application. Check that the application is installed and not damaged. You may need to check system permissions as well."
            
        case .applicationNotFound:
            return "The specified application could not be found. Verify the application name or bundle identifier and ensure the application is installed on the system."
            
        case .network:
            return "Network communication failed. Check your internet connection and firewall settings."
            
        case .parsing:
            return "Failed to parse data. The format may be invalid or unexpected."
            
        case .timeout:
            return "The operation timed out. The system might be under heavy load or the operation may be too complex."
...
extension NSError {
    ...
    var asMacMCPError: MacMCPErrorInfo? {
        ...
        if domain.hasPrefix(MacMCPErrorDomain) {
            ...
            let categoryString = domainComponents.count > 1 
                ? domainComponents[1]
                : "unknown"
            
            // Get category
            let category = MacMCPErrorCategory(rawValue: categoryString.capitalized) 
                ?? .unknown
