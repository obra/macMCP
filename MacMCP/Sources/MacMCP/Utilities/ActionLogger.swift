// ABOUTME: This file implements an action logging system for tracking UI operations.
// ABOUTME: It provides logging, filtering, and retrieval of user interaction history.
...
public actor ActionLogService {
    ...
    public func logAction(_ entry: ActionLogEntry) {
        ...
        if entry.success {
            logger.info("[\(entry.category)] \(entry.action)", metadata: [
                "targetId": "\(entry.targetId ?? "none")",
                "details": "\(entry.details)"
            ])
        } else {
            logger.error("[\(entry.category)] \(entry.action) - FAILED", metadata: [
                "targetId": "\(entry.targetId ?? "none")",
                "details": "\(entry.details)",
