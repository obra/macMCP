...
public enum ResourceURIParser {
    ...
    if !uri.contains("://") {
      ...
      if pathComponents.count > 1, let queryString = pathComponents.last {
        ...
        for item in queryItems {
          ...
          if keyValue.count == 2 { queryParams[String(keyValue[0])] = String(keyValue[1]) }
        }
      }
      return ResourceURIComponents(scheme: "macos", path: path, queryParameters: queryParams)
    }
  ...
  public static func parseQueryParameters(_ params: [String: String]) -> ResourceQueryParameters {
    ...
    // Parse limit
    if let limitStr = params["limit"], let limit = Int(limitStr) { result.limit = limit }
    // Parse interactable
...
public struct ResourceURIComponents {
  ...
  /// Path components
  public var pathComponents: [String] { ResourceURIParser.pathComponents(from: path) }
