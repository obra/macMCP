...
extension ResourceHandler {
  ...
  public func canHandle(uri: String) -> Bool {
    // Parse the URI
    guard let components = try? ResourceURIParser.parse(uri) else { return false }
    ...
    if patternComponents.count != uriComponents.count { return false }
...
public class ResourceRegistry: @unchecked Sendable {
  ...
  public func register(_ handler: ResourceHandler) { handlers.append(handler) }
  ...
  public func listResources(cursor: String? = nil, limit: Int? = nil) -> (
    [ListResources.Resource], String?
  ) {
