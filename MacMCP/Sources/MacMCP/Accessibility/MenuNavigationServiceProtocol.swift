...
public protocol MenuNavigationServiceProtocol: Actor {
  ...
  func getCompleteMenuHierarchy(bundleId: String, maxDepth: Int, useCache: Bool) async throws
    -> MenuHierarchy
  ...
  func getMenuDetails(bundleId: String, menuPath: String, includeSubmenus: Bool) async throws
    -> CompactMenuItem
  ...
  /// - Returns: Boolean indicating success
  func activateMenuItemByPath(bundleId: String, menuPath: String) async throws -> Bool
}
