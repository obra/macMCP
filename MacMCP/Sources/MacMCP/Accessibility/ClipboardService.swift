// ABOUTME: Implements clipboard service operations for macOS
// ABOUTME: Provides concrete implementations for reading and writing clipboard data
...
public actor ClipboardService: ClipboardServiceProtocol {
    ...
    public func getClipboardFiles() async throws -> [String] {
        ...
        if let urls = pasteboard.readObjects(forClasses: [NSURL.self], options: nil) as? [URL] {
            let fileURLs = urls.filter { $0.isFileURL }.map { $0.path }
            if !fileURLs.isEmpty {
                return fileURLs
            }
        }
    ...
    public func setClipboardFiles(_ paths: [String]) async throws {
        ...
        if !missingFiles.isEmpty {
            let fileNames = missingFiles.map { $0.lastPathComponent }.joined(separator: ", ")
            throw createClipboardError(
