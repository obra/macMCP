// ABOUTME: MCPElementPrinter.swift
...
class MCPElementPrinter {
  ...
  func formatElement(
    _ element: MCPUIElementNode,
    showColor: Bool = true,
    showAllData _: Bool = true,
    highlightPath: Bool = false,
  ) -> String {
    var output = ""
    ...
    if highlightPath {
      ...
      if let fullPath = element.fullPath {
        if showColor {
          output +=
            "   \(TerminalColor.boldGreen.rawValue)Path: \(fullPath)\(TerminalColor.reset.rawValue)\n"
        } else {
      ...
      // Otherwise generate a synthetic path if possible
        ...
        if showColor {
          output +=
            "   \(TerminalColor.yellow.rawValue)Path (generated): \(syntheticPath)\(TerminalColor.reset.rawValue)\n"
        } else {
