// ABOUTME: This is a simple utility to monitor keyboard events for testing keyboard interaction tools.
// ABOUTME: It displays all keyboard events in real-time, showing key codes and names for easy reference.
...
struct KeyCodeMap {
    ...
    static func name(for keyCode: UInt8) -> String {
        return keyNames[Int(keyCode)] ?? "unknown(\(keyCode))"
    }
...
struct KeyEvent: CustomStringConvertible {
    ...
    var description: String {
        return String(format: "%8.3f | %5d | %-10s | %s",
                      timestamp,
                      keyCode,
                      keyName,
                      type.rawValue)
    ...
    mutating func run() throws {
        ...
        if log {
            ...
            if let outputPath = output {
                ...
                logPath = FileManager.default.temporaryDirectory.appendingPathComponent("keymon-log-\(dateString).txt")
            }
            ...
            if let logFile = logFile, let logPath = logPath {
        ...
        while true {
            ...
            if amountRead > 0 {
                ...
                if timestamp {
                    ...
                    logEntryText = String(format: "%5d | %-10s | %s",
                                          event.keyCode,
                                          event.keyName,
                                          event.type.rawValue)
                }
                ...
                if !downOnly {
                    ...
                    if timestamp {
                        ...
                        upLogEntryText = String(format: "%5d | %-10s | %s",
                                              upEvent.keyCode,
                                              upEvent.keyName,
