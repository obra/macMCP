...
func getKeyName(for keyCode: UInt8) -> String {
  keyMap[Int(keyCode)]?.keyName ?? "unknown(\(keyCode))"
}
...
let logPath = FileManager.default.temporaryDirectory.appendingPathComponent(
  "cli-keyboard-monitor-\(dateString).log")
