// ABOUTME: OutputFormatters.swift
...
class PlainTextFormatter: OutputFormatter {
  ...
  init(showColor: Bool = true, showDetails: Bool = false) {
    ...
    treeVisualizer = TreeVisualizer(options: options)
  }

  func format(_ rootElement: UIElementNode, withFilters: [String: String]) -> String {
    treeVisualizer.visualize(rootElement, withFilters: withFilters)
  }
}
...
class XMLFormatter: OutputFormatter {
  ...
  private func formatElementAsXML(
    ...
    output +=
      indent + "  <state enabled=\"\(element.isEnabled)\" clickable=\"\(element.isClickable)\" />\n"
      ...
      for key in sortedKeys {
        if let value = element.attributes[key] {
          ...
          if !valueString.isEmpty {
            output +=
              indent + "    <attribute name=\"\(escapeXML(key))\">\(valueString)</attribute>\n"
          }
        }
      }
